...
```bash
curl -X POST http://localhost:8001/api/v1/scenario/generate   -H "Content-Type: application/json"   -d '{"topic": "sécurité incendie", "custom_prompt": "Ajouter des consignes spécifiques pour les bureaux"}'
```

### Récupérer un scénario existant

```bash
curl http://localhost:8001/api/v1/scenario/<session_id>
```

### Poser une question à un scénario

```bash
curl -X POST http://localhost:8001/api/v1/scenario/<session_id>/question   -H "Content-Type: application/json"   -d '{"question": "Quelles sont les procédures d'évacuation ?"}'
```
